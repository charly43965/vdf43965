 ...
 *
 * Run once from the command line: php import_france.php
 *
 * Requirements:
 *   - PHP 8.1+
 *   - cURL extension enabled
 *   - cert.crt in the same directory as PocketBase.php
 */
...
try {
    ...
    $locations = new LocationManager($pocketbase);
} catch (Exception $e) {
...
function geoFetch(string $url): array {
    ...
    curl_setopt_array($curl, [
        ...
        CURLOPT_TIMEOUT        => 30, // Increased for large payloads
...
function slugify(string $text): string {
    ...
    $text = str_replace(
        // Replace special characters with standard UTF-8 equivalents
        ["à","â","ä","é","è","ê","ë","î","ï","ô","ö","ù","û","ü","ç","æ","œ","ñ","'"],
        ["a","a","a","e","e","e","e","i","i","o","o","u","u","u","c","ae","oe","n","-"],
        $text,
    );
...
foreach ($regionsJSON as $r) {
    $slug = slugify($r['nom']);
    try {
        $record = $locations->createRegion($r['code'], $r['nom'], $slug);
        $regionsArray[$r['code']] = $record['id'];
        console_log("Added region {$r['nom']}.");
    } catch (RuntimeException $e) {
        console_log("Could not add region {$r['nom']}: {$e->getMessage()}");
    }
...
foreach ($departmentsJSON as $d) {
    $regionId = $regionsArray[$d['codeRegion']] ?? null;

    if (!$regionId) {
        console_log("Could not find region {$d['codeRegion']} for department {$d['nom']}, skipping.");
        continue;
    }
    ...
    try {
        $record = $locations->createDepartment($regionId, $d['code'], $d['nom'], $slug);
        $departmentsArray[$d['code']] = $record['id'];
        console_log("Added department {$d['nom']}.");
    } catch (RuntimeException $e) {
        console_log("Could not add department {$d['nom']}: {$e->getMessage()}");
    }

    usleep(RATE_LIMIT_MS / 2); // Faster inside departments
}

//==============
// Cities
// (Fetching per department)
//==============
$cityCount = 0;
$citySlugs = []; // To track duplicates
...
foreach ($departmentsArray as $departmentCode => $departmentId) {
    ...
    foreach ($cities as $c) {
        $gpsLatitude    = $c['centre']['coordinates'][1] ?? null;
        $gpsLongitude   = $c['centre']['coordinates'][0] ?? null;
        $postalCode     = $c['codesPostaux'][0] ?? '';  // Take first postal code

        $baseSlug = slugify($c['nom']);
        $slug = isset($citySlugs[$baseSlug]) ? $baseSlug . '-' . strtolower($c['code']) : $baseSlug; // Append code_insee if collision
        $citySlugs[$slug] = true;
        ...
        try {
            ...
            $cityCount++;
            console_log("Added {$c['nom']} to $departmentCode.");
        } catch (RuntimeException $e) {
            console_log("City {$c['nom']} ({$c['code']}): " . $e->getMessage());
        }
    }
    usleep(RATE_LIMIT_MS);
}
...
foreach ($prefectures as $departmentCode => $inseeCode) {
    ...
    $city = $locations->getCityByInsee($inseeCode);
    ...
    try {
        $locations->updateDepartment($departmentId, prefectureId: $city['id']);
    } catch (RuntimeException $e) {
